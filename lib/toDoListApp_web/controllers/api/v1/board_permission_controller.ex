defmodule ToDoListAppWeb.Api.V1.BoardPermissionController do
  ...
  def index(conn, %{"board_id" => board_id}) do
    case BoardContext.get_board!(board_id) do
      {:ok, _board} ->
        with {:ok, board_permissions} <- {:ok, BoardContext.list_board_permission(board_id)} do
          conn
          |> put_status(:ok)
  ...
  def create(conn, %{"board_id" => board_id, "board_permission" => board_permission_params}) do
    case BoardContext.get_board!(board_id) do
      {:ok, _board} ->
        with {:ok, %BoardPermission{} = board_permission} <- BoardContext.create_board_permission(board_permission_params) do
          conn
          |> put_status(:created)
          |> put_resp_header("location", Routes.api_v1_board_board_permission_path(conn, :show, board_permission.board_id, board_permission.board_permission_id))
